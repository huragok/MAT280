...
\end{lstlisting}
    ...
    \problemAnswer{
      ...
      plotted in Figure~\ref{fig:dimension_reduction}. As we can see, the
      Johnson-Lindenstrauss (JL) method, the Fast Johnson-Lindenstrauss (FJL)
      methohd and the random selection (RS) method has approximatedly the same
      performance in terms of accuracy. As $k\rightarrow 1024$, the error of all
      the three methods approaches 0.
      
      \begin{figure}[htb]
        \centering
        \includegraphics[width=0.5\columnwidth]{figs/dimension_reduction.pdf}
